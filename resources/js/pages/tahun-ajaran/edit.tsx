...
export default function Edit({ tahunAjaran }: EditProps) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        kode_tahun_ajaran: tahunAjaran.kode_tahun_ajaran,
        tahunawal: tahunAjaran.tahunawal,
        tahunakhir: tahunAjaran.tahunakhir,
        is_active: tahunAjaran.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(tahunAjaranRoutes.show({ tahun_ajaran: tahunAjaran.id }).url);
    };

    const isActivatingNow = !tahunAjaran.is_active && data.is_active;
                            ...
                            <div className="space-y-2">
                                ...
                                {isActivatingNow && (
                                    <p className="text-sm text-yellow-600">
                                        ⚠️ Mengaktifkan tahun ajaran ini akan menonaktifkan tahun ajaran lain yang sedang aktif
                                    </p>
                                )}
                            </div>
                        </CardContent>
